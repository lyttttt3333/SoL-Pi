...
The workflow runs on Windows Server 2022 and Ubuntu 22.04 with Node 22.23.2, against Pi 0.84.2 and 0.85.1. It compares upstream main `2b791687` with the exact source/test patch from PR head `788d8c1e`, including the POSIX-only test correction. Both patched files are checked against the exact PR content. Installation uses locked dependencies with lifecycle scripts disabled; the 0.84.2 fixture adjusts only package pins, lockfile, and guide version strings.
...
The Action Fusion path suite also runs separately so its result is visible independently of existing Windows suite failures. Only result JSON and logs are uploaded, not the isolated HOME or npm cache.
