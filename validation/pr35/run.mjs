...
requirePass(run('fetch-pr', 'git', ['fetch', '--no-tags', 'https://github.com/NVlabs/SoL-Pi.git', '7d7f082eeac2530e4e982b947e28af221a19b2e1']), 'fetch');
...
writeFileSync(join(out, 'environment.json'), JSON.stringify({ platform: process.platform, node: process.version, shell, versions, base: '2b791687a489a1d24da816cf1634d8ae1d36befd', head: '7d7f082eeac2530e4e982b947e28af221a19b2e1' }, null, 2));
...
for (const variant of ['baseline', 'fixed']) {
  if (variant === 'fixed') {
    const patch = run('read-pr-patch', 'git', ['diff', '7d7f082eeac2530e4e982b947e28af221a19b2e1^', '7d7f082eeac2530e4e982b947e28af221a19b2e1', '--', 'src/sol-pi/extensions/action-fusion/file-queue.ts', 'tests/action-fusion-paths.test.ts']);
    requirePass(patch, 'read patch');
    requirePass(run('apply-pr-patch', 'git', ['apply', '--whitespace=error', '-'], { input: patch.stdout }), 'apply patch');
    const expected = run('pr-source', 'git', ['show', '7d7f082eeac2530e4e982b947e28af221a19b2e1:src/sol-pi/extensions/action-fusion/file-queue.ts']);
